...
function EventCard(props) {
  ...
  return (
    <div className="event-card" key={props.data._id}>
      <img
        src={props.data.banner ? props.data.banner : DefaultImage}
        className="event-banner"
        alt={props.data.eventName}
        onClick={() => openEvent(props.data)}
      />
      ...
      {props?.data?.eventMode === "Online" ? (
        <span className="event-full-address">
          {getOnlineName(props.data.onlineURL)}
        </span>
      ) : (
        <span className="event-full-address">
          {props.data.city}
      ...
      {props.edit ? (
        <button className="edit-btn" onClick={(e) => editEvent(props.data._id)}>
          Edit
        </button>
      ) : (
        <h3 style={{ marginTop: -25, display: "inline-block", float: "right" }}>
          {props.data.entryMode === "free"
            ? "Free"
            : "₹" +
              CommonService.numberWithCommas(
                CommonService.getAmount(props.data.ticketCategory)
              )}
        </h3>
      )}
    </div>
