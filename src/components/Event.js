...
import ZoomBanner from "../assets/images/zoom.png";
import TeamsBanner from "../assets/images/teams.jpg";
import MeetBanner from "../assets/images/meets.jpg";
import {
  EmailShareButton,
  FacebookShareButton,
...
function Event(props) {
  ...
  const IsAlreadyAdded = (id) => {
    return (
      selectedTickets.find((a) => a.id === id) &&
      selectedTickets.find((a) => a.id === id)?.ticket_count > 0
    );
  };
  const getCount = (id) => {
  ...
  const GetTotalPrice = () => {
    if (selectedTickets.length > 0) {
      ...
      for (item in selectedTickets) {
        sum +=
          selectedTickets[item].amount * selectedTickets[item].ticket_count;
      }
      return sum;
    } else {
      return 0;
  ...
  const sendBookingEmail = () => {
    ...
    if (event.entryMode === "free") {
      ...
      CommonService.sendBookingEmail(selectedTickets, user, event).then(
        (res) => {
          setOpen(false);
          CommonService.showToast(res.message);
        }
    ...
    <div>
        ...
        <div>
          <div className="event-container">
            <div className="left-card">
              <img
                alt={event.eventName}
                src={event.banner || Placeholder}
                className="event-banner"
              />
              {/* {event.avlSeats && (
                <span className="avl-seats">
                  ...
                  <p className="event-location">
                    ...
                    <span className="location-text">
                      {event.eventMode === "Online"
                        ? CommonService.getOnlineName(event.onlineURL)
                        : CommonService.trimString(event.full_address, 63)}
                    </span>
                  </p>
                </div>
                <div style={{ display: "flex", gap: 20 }}>
                  <h3 style={{ marginTop: 10 }}>
                    {event.entryMode === "free"
                      ? "Free"
                      : "₹" +
                        CommonService.numberWithCommas(
                          CommonService.getAmount(event.ticketCategory)
                        )}
                    {event.avlSeats && (
                      <span className="center avt-count">
                        {event.avlSeats} seats available
                      </span>
                    )}
                  </h3>
                  <Button
            ...
            <div className="right-card">
              <div style={{ borderBottom: "1px solid #eee", paddingBottom: 0 }}>
                <div style={{ height: 400 }}>
                  {event?.eventMode === "Online" ? (
                    <>
                      <div className="onlineBannerDiv">
                        <img
                          alt="Online meeting banner"
                          className="onlineBanner"
                          src={getOnlineImg(event?.onlineURL)}
                        />
                      </div>
                    </>
                  ) : (
              ...
              <div className="border" style={{ padding: 10 }}>
                ...
                <div className="share-icons">
                  <FacebookShareButton
                    url={
                      "https://cookitabhi.web.app/dish/5f019b65a3bc3a265c875c3d"
                    }
      ...
      >
        {event?.entryMode === "free" ? (
              ...
              <DialogContentText id="alert-dialog-slide-description">
                ...
                {freeTicket > 0 ? (
                  <div className="count-icons">
                    <RemoveIcon
                      onClick={() => setFreeTicket(freeTicket - 1)}
          ...
          <>
            <DialogTitle id="alert-dialog-slide-title">
              {"Select seat and Category"}
            </DialogTitle>
            <DialogContent>
              <DialogContentText id="alert-dialog-slide-description">
                {event?.ticketCategory?.map((cat, i) => {
                  return (
                    <div className="cat-div" key={i}>
                      <p className="cat-name">{cat.category}</p>
                      <span className="cat-price">₹ {cat.amount}</span>
                      <br />
                      {cat.description && (
                      ...
                      {IsAlreadyAdded(cat.id) ? (
                        <div className="count-icons">
                          <RemoveIcon
                            onClick={() => removeFromList(cat.id)}
                            style={{ float: "left" }}
                          />
                          <span>{getCount(cat.id)}</span>
                          <AddIcon
                            onClick={() => addToList(cat.id)}
                            style={{ float: "right" }}
                          />
                        </div>
                      ) : (
